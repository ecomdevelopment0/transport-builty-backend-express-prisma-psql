export class EntityConstants {
  public static readonly ADMINS = "admins";
  public static readonly BLACKLISTS = "blacklists";
  public static readonly BUYERS = "buyers";
  public static readonly CATEGORIES = "categories";
  public static readonly FIRMS = "firms";
  public static readonly PAYMENT_COLLECTIONS = "paymentCollections";
  public static readonly PAYMENT_MODES = "paymentModes";
  public static readonly PERMISSIONS = "permissions";
  public static readonly PREFIXES = "prefixes";
  public static readonly PRODUCTS = "products";
  public static readonly RENTALS = "rentals";
  public static readonly RENTAL_PRODUCTS = "rentalProducts";
  public static readonly ROLES = "roles";
  public static readonly SELLERS = "sellers";
  public static readonly SUBSCRIPTIONS = "subscriptions";
  public static readonly TERMS_AND_CONDITIONS = "termsAndConditions";
}
